...
<body>
	<div class="wrap">
		<ul class="nav nav-tabs">
			...
			<li><a href="<?php echo U('Textbook/addtextbook');?>">新增教科书</a></li>
			<li class="active"><a href="<?php echo U('Textbook/addtextbook',array('edit_id'=>$id));?>">编辑教科书</a></li>
		</ul>
<form method="post" class="form-horizontal js-ajax-form" action="<?php echo U('Textbook/edit_post');?>">
      <fieldset>
        <div class="control-group">
          ...
          <div class="controls">
              ...
              <select type="text" name="subject" >
                ...
                <?php if(is_array($subject)): foreach($subject as $key=>$v): ?><option value="<?php echo ($v['title']); ?>" <?php if($v["title"] == $res["subject"] ): ?>selected<?php endif; ?> ><?php echo ($v[title]); ?></option><?php endforeach; endif; ?>
              </select>
        ...
        <div class="control-group">
          ...
          <div class="controls">
              ...
              <select type="text" name="press" >
                <option value="">请选择</option>
                <?php if(is_array($press)): foreach($press as $key=>$v): ?><option value="<?php echo ($v['title']); ?>" <?php if($v["title"] == $res["press"] ): ?>selected<?php endif; ?> ><?php echo ($v[title]); ?></option><?php endforeach; endif; ?>
              </select>
        ...
        <div class="control-group">
          ...
          <div class="controls">
              ...
              <select type="text" name="grade">
                <option value="">请选择</option>
                <?php if(is_array($grade)): foreach($grade as $key=>$v): ?><option value="<?php echo ($v['title']); ?>上学期" <?php if($v['title']."上学期"==$res['grade']){ ?>selected<?php } ?> ><?php echo ($v[title]); ?>上学期</option>
                  <option value="<?php echo ($v['title']); ?>下学期" <?php if($v['title']."下学期"==$res['grade']){ ?>selected<?php } ?> ><?php echo ($v[title]); ?>下学期</option><?php endforeach; endif; ?>
              </select>
    ...
    <button type="button"  onclick="add_zhang()" class="btn btn-link" >添加章</button>
      ...
      <div id="addchapter">
      <input type="hidden" name="i" value="<?php if(count($zhang)){ echo count($zhang)+1; }else{ echo 1; }?>">
      <input type="hidden" name="t_id" value="<?php echo ($res[id]); ?>">
        <?php if($zhang){ foreach($zhang as $k=>$v){ if($v){ ?>
          <div class="row add_zhang rep" id="zz<?php echo ($k+1); ?>">
          章标题：<input type="text" name="zhang<?php echo ($k+1); ?>" value="<?php echo ($v[title]); ?>" > 
          <button type="button" onclick="add_jie('<?php echo ($k+1); ?>')" id="z<?php echo ($k+1); ?>" class="btn btn-link">添加节</button>
          <button type="button" class="btn btn-danger" onclick="del_div($(this))" >删除章</button>
            <?php if($v[jie]){ foreach($v[jie] as $key=>$val){ if($val){ ?>
              <div id="j<?php echo ($k+1); ?>" class="row add_jie rej"><div>节标题：<input type="text" name="jie<?php echo ($k+1); ?>[]" value="<?php echo ($val[title]); ?>" ><button type="button" class="btn btn-danger bu" onclick="del_div2($(this))" >删除节</button></div></div>
             <?php  } } } ?>
          </div>
      <?php } } } ?>
      </div>
      ...
      <div class="form-actions">
        <button type="subimt" class="btn btn-primary js-ajax-submit"><?php echo L('SAVE');?></button>
