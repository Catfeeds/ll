...
<meta charset="utf-8">
<title>来龙教育 <?php echo L('ADMIN_CENTER');?></title>
...
<![endif]-->
<link rel="stylesheet" href="/ll/public/simpleboot/themes/<?php echo C('SP_ADMIN_STYLE');?>/simplebootadminindex.min.css?">
<!--[if lte IE 8]>
	<link rel="stylesheet" href="/ll/public/simpleboot/css/simplebootadminindex-ie.css?" />
<![endif]-->
<style>
...
<?php foreach($submenus as $menu){ ?>
					<li id="<?php $menu_name=L($menu['lang']); echo $menu_name=$menu['lang']==$menu_name?$menu['name']:$menu_name; ?>">
...
<body style="min-width:900px;" screen_capture_injected="true">
		...
		<div class="navbar-inner">
			<div class="container-fluid">
				<a href="<?php echo U('index/index');?>" class="brand"> <small> 
						来龙教育 <?php echo L('ADMIN_CENTER');?>
				</small>
				</a>
				<div class="pull-left nav_shortcuts" >
					
					<a class="btn btn-small btn-warning" href="/ll/" title="<?php echo L('WEBSITE_HOME_PAGE');?>" target="_blank">
				...
				<ul class="nav simplewind-nav pull-right">
					<li class="light-blue">
						<a data-toggle="dropdown" href="#" class="dropdown-toggle">
							...
							<?php else: ?>
								<img class="nav-user-photo" width="30" height="30" src="/ll/admin/themes/lailong/Public/assets/images/logo-18.png" alt="<?php echo ($admin["user_login"]); ?>"><?php endif; ?>
	...
	<script src="/ll/admin/themes/lailong/Public/assets/js/index.js"></script>
	<script type="text/javascript">
		//意见反馈管理
			function updateOpinion (){
				$.ajax({
				type: "POST",
				url: "<?php echo U('Public/updateOpinion');?>",
				dataType : 'json',
				success: function(res){
					if(res){
						$("#yjfk").remove();
				       $("#意见反馈管理").children().append('<span id="yjfk" style="margin-left:20px;background-color:#FF0000;color: #FFFFFF;">'+res+'<span>');
					}  
				},
				});
			}
		//报名管理
			function updateSign (){
				$.ajax({
				type: "POST",
				url: "<?php echo U('Public/updateSign');?>",
				dataType : 'json',
				success: function(res){
					if(res){
						$("#bmgl").remove();
				       $("#报名管理").children().append('<span id="bmgl" style="margin-left:20px;background-color:#FF0000;color: #FFFFFF;">'+res+'<span>');
					}  
				},
				});
			}
		//意向管理
			function updateIntention (){
				$.ajax({
				type: "POST",
				url: "<?php echo U('Public/updateIntention');?>",
				dataType : 'json',
				success: function(res){
					if(res){
						$("#yxgl").remove();
				       $("#意向管理").children().append('<span id="yxgl" style="margin-left:20px;background-color:#FF0000;color: #FFFFFF;">'+res+'<span>');
					}  
				},
				});
			}
		//小孩管理
			function updateChildren (){
				$.ajax({
				type: "POST",
				url: "<?php echo U('Public/updateChildren');?>",
				dataType : 'json',
				success: function(res){
					if(res){
						$("#xhgl").remove();
				       $("#孩子管理").children().append('<span id="xhgl" style="margin-left:20px;background-color:#FF0000;color: #FFFFFF;">'+res+'<span>');
					}  
				},
				});
			}	
		//测试结束，可行
		// setInterval(updateOpinion, 5000); //5秒钟执行一次
		// setInterval(updateSign, 5000);
		// setInterval(updateIntention, 5000);
		// setInterval(updateChildren, 5000);
		$(function(){
			$("#意见反馈管理").click(function(){
				$("#yjfk").remove();
			});
			$("#报名管理").click(function(){
				$("#bmgl").remove();
