...
class MineController extends MemberbaseController {
	...
	function my(){
		...
		M("user")->where("id=".$userid)->save(array("last_login_time"=>date("Y-m-d H:i:s")));
		M('login_record')->add(array('user_id'=>$userid,'login_time'=>date('Y-m-d H:i:s'),'createtime'=>date('Y-m-d H:i:s'),'app_version'=>'weixin','device_name'=>''));
		//查询孩子
...
//		$this->assign('cid',$id);//小孩id
	...
	function do_member(){
	    ...
	    if(!empty($editid)){
			...
			M('user')->where("phone=$phone")->save(array('name'=>$username));
	        $this->ajaxReturn(array("0"=>"101","1"=>'用户认证中'));
	    }
	}
	...
	function lineitem() {
		$id=$_REQUEST['id'];
		if($id){
		    $field="b.*,a.state,a.cancel_state,a.order_id,a.createtime,a.note,a.id as oid,c.name,d.name as cname,b.integral as bintegral ,b.start_time as time,cr.title as classroom,cr.address,cr.phone as classroomphone";
		...
		$type_state=array("0"=>"订单已提交","1"=>"支付成功","3"=>"已完成","4"=>"已取消","5"=>"申请取消订单被拒");
	...
	//申请取消订单
	function black_order(){
		$rest = M('enrollment')->where(['id'=>I('post.orderid')])->find();
		if($rest['note'] != null){
			$str = substr($rest['note'],0,strpos($rest['note'],"|"));
			$strall = $str.'|订单取消中，取消理由为：'.I('post.content');
		}else{
			$strall = '|订单取消中，取消理由为：'.I('post.content');
		}
		$data = ['note'=>$strall,'cancel_state'=>'1'];
		$result = M('enrollment')->where(['id'=>I('post.orderid')])->save($data);
		if($result){
			$code = ['0'=>'101','1'=>'申请成功'];
		} else{
			$code = ['0'=>'','1'=>'申请失败'];
	...
	function collection(){
	    ...
	    $collect=M('collection')->field("c.id,c.title,c.cover,c.start_time,t.name as teacher")->table(C('DB_PREFIX')."collection as col")->join(C('DB_PREFIX')."course as c on c.id = col.course_id",'left')
	    ...
	    foreach ($collect as $c=>$co){
	        ...
	        $collect[$c][cover] =__ROOT__.'/data/upload/avatar/'.$co[cover];
	...
	function my_message(){
	    ...
	    ->join(C('DB_PREFIX')."course as c on c.id = n.course_id",'left')->where("user_id =$user or user_id=0 and n.createtime >'$u[createtime]'")->count();
	    $n=M('news')->table(C('DB_PREFIX')."news as n")->field("n.title,n.content,n.createtime,n.icon,c.id as course_id")
	    ->join(C('DB_PREFIX')."course as c on c.id = n.course_id",'left')->order(array("n.createtime"=>DESC))
	    ->limit("0,".$num)->where("((user_id =$user) or user_id =0) and n.createtime >'$u[createtime]'")->order("n.createtime desc")->select();
	    foreach($n as $k=>$v){
	...
	function sign(){
       ...
       if(M('check_in')->add(array('user_id'=>$user,'createtime'=>date('Y-m-d H:i:s',time()),'Integral'=>$ec[integral]))){
           if($ec){
               $data=array(
                   'user_id'=>$user,
                   'obtain_type'=>2,
	...
	function server(){
        ...
        $rest_tc= M('teacher')->count();
        $rest_t = M('teacher') ->limit("0,$num")->order(array("createtime"=>DESC))->select();
        if($result_c>$num){
        	$typea=1;
        }
        if($rest_c>$num){
	    ...
	    $this->assign("typec",$typec);
        $this->assign('list',$result);
        $this->assign('ilist',$rest);
        $this->assign('tlist',$rest_t);
		$this->display("/server");
	}
	function server_art() {
		$id = I ( 'get.id' );
		if(I('get.type') == 's'){
			$res=M('service')->alias('s')->join(C('DB_PREFIX')."h5 as h on h.id =s.h5_id",'left')->field("s.title ,h.content,h.url,h.createtime")
		      ->where("s.id = $id")->find();
		}elseif(I('get.type') == 'i'){
			$res=M('information')->alias('s')->join(C('DB_PREFIX')."h5 as h on h.id =s.h5_id",'left')->field("s.title ,h.content,h.url,h.createtime")
		      ->where("s.id = $id")->find();
		}
		if (!empty ( $res ['url'] )) {
   ...
   function up(){
	   $exif = exif_read_data($_FILES);//获取exif信息
		if (isset($exif['Orientation']) && $exif['Orientation'] == 6) {
			imagerotate($_FILES,-90,0);
		}
       $config = array (
      ...
      /*  $cutpath= C ( "UPLOADPATH" )."avatar/".$savename;
       $im = imagecreatefromjpeg($cutpath);
       $name = time() . rand(0,999999);
       $newname = $this->resizeImage($im,120,120,$name,'.jpg');   //压缩图片
       $savenames=explode(C ( "UPLOADPATH" )."avatar/",$newname); */
       //ajax返回数据
       if ($savename) {
   ...
   public function resizeImage($im,$maxwidth,$maxheight,$name,$filetype)
   {
       ...
       if(($maxwidth && $pic_width > $maxwidth) && ($maxheight && $pic_height > $maxheight))
       {
           ...
           if(function_exists("imagecopyresampled"))
           {
               ...
               $newsize = $newwidth >= $newheight ? $newheight : $newwidth;
               $new_x = ($newwidth - $newsize)/2;
               $new_y = ($newheight - $newsize)/2;
               $new_im = imagecreatetruecolor($newsize,$newsize);
               imagecopyresampled($new_im,$newim,0,0,$new_x,$new_y,$newwidth,$newheight,$newwidth,$newheight);
           }
           else
           {
               ...
               $new_x = ($newwidth - $newsize)/2;
               $new_y = ($newheight - $newsize)/2;
               $new_im = imagecreate($newsize,$newsize);
               imagecopyresized($new_im,$newim,0,0,$new_x,$new_y,$newwidth,$newheight,$newwidth,$newheight);
           }
   
           $name =  C ( "UPLOADPATH" ) . "avatar/".$name.$filetype;
   	  	 ...
   	  	 $str.='
   	  	 		<li style="padding-left: 5px;margin-bottom: 11px; border: none;" class="am-g am-list-item-desced am-list-item-thumbed am-list-item-thumb-left">
                            ...
                            <div class=" am-u-sm-9 am-list-main">
                                <h6  class="am-list-item-hd"><a class="" style="font-size: 1.6rem;color:black;"><a href="'.U("Mine/server_art?id=").$v['h5_id'].'&type='.$type.'">'.$v['title'].'</a></h6>
                                <a href="'.U("Mine/server_art?id=").$v['h5_id'].'&type='.$type.'">
   ...
   function integral_more(){
   	...
   	foreach($mes as $k=>$v){
   		...
   		$str.='
	            ...
	            <td style="float:left; margin-top: 6px; line-height: 25px;color: #969cb0;border-top: none;word-break:break-all; width:200px; overflow:auto; ">'.$v['content'].'</td>
