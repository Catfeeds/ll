...
<body>
	<div class="wrap">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="<?php echo U('course/editcourse',array('editid'=>$id));?>">编辑课程</a></li>
		</ul>
		<form method="post" class="form-horizontal js-ajax-form" action="<?php echo U('course/edit_post');?>">
			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php if(!empty($res[cover])): ?><img src="/ll/public/images/tv.gif" id='close2' style="margin-left: -13px;margin-top: -11px;position: relative;" />
							<div id="img2" style="width: 100px;height: 100px;background: url(/ll/data/upload/avatar/<?php echo ($res[cover]); ?>) no-repeat center; background-size: cover;"></div><?php endif; ?>
							<div id="add_img" <?php if(!empty($res[cover])): ?>style="display:none;"<?php endif; ?>>
								<input type="hidden" name="cover" value="<?php echo ($res["cover"]); ?>"> 
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php if(!empty($res[picture])): ?><img src="/ll/public/images/tv.gif" id='close3' style="margin-left: -13px;margin-top: -11px;position: relative;" />
							<div id="img3" style="width: 100px;height: 100px;background: url(/ll/data/upload/avatar/<?php echo ($res[picture]); ?>) no-repeat center; background-size: cover;"></div><?php endif; ?>
						<div id="add_img2"  <?php if(!empty($res[picture])): ?>style="display:none;"<?php endif; ?>>
							<input type="hidden" name="picture" value="<?php echo ($res["picture"]); ?>"> 
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" name="rated_number" value="<?php echo ($res["rated_number"]); ?>">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label"><?php echo L('SCORE');?></label>
					<div class="controls">
						<input type="text" name="integral" value="<?php echo ($res["integral"]); ?>">
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<select name="teacher_id">
							<?php if(is_array($teacher)): foreach($teacher as $key=>$t): ?><option value="<?php echo ($t["id"]); ?>"<?php if($res[teacher_id] == $t[id]): ?>selected<?php endif; ?>><?php echo ($t["name"]); ?></option><?php endforeach; endif; ?>
						</select>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<select name="classroom_id">
							<?php if(is_array($classroom)): foreach($classroom as $key=>$t): ?><option value="<?php echo ($t["id"]); ?>"<?php if($res[classroom_id] == $t[id]): ?>selected<?php endif; ?>><?php echo ($t["address"]); ?></option><?php endforeach; endif; ?>
						</select>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						...
						<input type="hidden" name="h5_id" value="<?php echo ($res["h5_id"]); ?>" />
					</div>
				</div>
				<div class="control-group">
					<label class="control-label"><?php echo L('STARTIME');?></label>
					<div class="controls">
						<input type="text"placeholder="请输入日期..." name="start_time" class="input length_2" value="<?php echo ($res["start_time"]); ?>" autocomplete="off" onClick="WdatePicker()" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss'})">
					</div>
				</div>
			</fieldset>
...
$('fieldset').on('click','#close2',function(){
	var img2 = $('#img2').next().children('input').val();
	 $.post('<?php echo U("course/deleteicon");?>', {
		 img2: img2
		},
		function(data) {
		}
