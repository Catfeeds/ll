...
<body>
	<div class="wrap">
		...
		<form method="post" class="form-horizontal js-ajax-form" action="<?php echo U('order/changestate');?>">
			...
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<?php if($list[state] == 0): ?><option value="0" selected>未支付</option>
							...
							<?php elseif($list[state] == 3): ?>
								<option value="3"  selected><?php if($list[state] == 3): ?>已完成<?php else: ?>完成<?php endif; ?></option>
							<?php elseif($list[state] == 4): ?>
								<option value="4"  selected><?php if($list[state] == 4): ?>已取消<?php else: ?>取消<?php endif; ?></option><?php endif; ?>
						</select>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="control-group" <?php if($list[cancel_state] == 0): ?>style="display:none;"<?php endif; ?>>
						<label class="control-label"><?php echo L('CANCELSTATE');?></label>
						<div class="controls">
							<?php if($list[cancel_state] == 1 && $list[state] != 4): ?>订单取消申请中<?php elseif($list[cancel_state] == 2 && $list[state] != 4): ?>订单取消被拒绝<?php elseif($list[state] == 4): ?>订单已取消<?php endif; ?>
						</div>
					</div>
					<div class="control-group Xx" >
						...
						<div class="controls">
							...
							<textarea id="desc" name="desc"style="width:300px;height:100px;"><?php echo ($list["note"]); ?></textarea>
						</div>
					</div>
