...
if (File_exists(ROOT_PATH . "api/config/config.php")) {
	require_once(ROOT_PATH . 'api/config/config.php');
}
$json = new JSON;
$nick_name = isset($_REQUEST['nick_name'])? $_REQUEST['nick_name']:'';
$wbuid = isset($_REQUEST['wbuid'])? $_REQUEST['wbuid']:'';
$username = isset($_REQUEST['wbuid'])? $_REQUEST['wbuid']:'';
$avatar = isset($_REQUEST['avatar'])? $_REQUEST['avatar']:'';
if(empty($wbuid)) {
    $result = array("code"=>"400", 'data'=>'缺少必要的参数wbuid');
...
if (empty($_REQUEST['avatar']) )
{
 ...
 $sql = "SELECT *  FROM " . $ecs->table('users') . " WHERE `uid`='" . $wbuid . "'";
 $userData = $db->getRow($sql);
$password='111111';
 if($userData == false) {
 	 // 创建用户
 	 $result = $user->add_user($username, $password, $username . "@weibo.com");
 	 if ($result == false) {
 	 	 ...
 	 	 if (!empty($userdata['user_id']) ) {
 	 	 	 // 更新 nick_name ,头像等字段
 	 	 	 $token = md5($wbuid);
 	 	 	 $sql = "update " .  $ecs->table('users') . "set nick_name='" . $nick_name . "', avatar = '" . $avatar . "',uid='" . $wbuid . "' where user_id=" . $userdata['user_id'];
 	 	 	 $db->query($sql);
 	 	 } else {
	 ...
	 $token=md5($wbuid . date("Y-M-D H:m:s"));
	 if($session == false ) {
	 	$session_record['user_id']= $userData['user_id'];
		$session_record['token']= $token;
		$session_record['times']= 1;
