...
$username = isset($_REQUEST['unionId'])? $_REQUEST['unionId']:'';
$avatar = isset($_REQUEST['avatar'])? $_REQUEST['avatar']:'';
if (empty($_REQUEST['unionId']) )
{
    $result = array("code"=>"400", 'data'=>'缺少必要的参数unionId');
...
if (empty($_REQUEST['avatar']) )
{
 ...
 $sql = "SELECT *  FROM " . $ecs->table('users') . " WHERE `unionId`='" . $unionId . "'";
 $userData = $db->getRow($sql);
 $password='111111';
 if($userData == false) {
 	 // 创建用户
 	 $result = $user->add_user($username, $password, $username . "@wx.com");
 	 if ($result == false) {
 	 	 ...
 	 	 if (!empty($userdata['user_id']) ) {
 	 	 	 // 更新 nick_name ,头像等字段
 	 	 	 $token = md5($unionId);
 	 	 	 $sql = "update " .  $ecs->table('users') . "set nick_name='" . $nick_name . "', avatar = '" . $avatar . "',unionId='" . $unionId . "' where user_id=" . $userdata['user_id'];
 	 	 	 $db->query($sql);
 	 	 } else {
	 ...
	 $token=md5($unionId . date("Y-M-D H:m:s"));
	 if($session == false ) {
	 	$session_record['user_id']= $userData['user_id'];
		$session_record['token']= $token;
		$session_record['times']= 1;
	 ...
	 $user_info =array("user_name"=>$username, "nick_name"=>$nick_name, "user_name"=>$username,"user_id"=>$userData['user_id'],"token"=>$token);
	 //检查是否是当天第一次登陆
