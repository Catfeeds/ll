 ...
 */       //已完成
   ...
   public function mymessage(){
       ...
       $pagec=$_REQUEST['pagec'];
       $start=($pagec-1)*$pagesize;
       $s=M('news')->where("user_id = 0")->order("createtime desc")->limit($start.','.$pagesize)->select();
       $n=M('news')->where("user_id = '$user'")->order("createtime desc")->limit($start.','.$pagesize)->select();
       foreach($s as $i=>$c){
       ...
       $arrSort = array();
       foreach($n AS $uniqid => $row){
           foreach($row AS $key=>$value){
               $arrSort[$key][$uniqid] = $value;
           }
       }
       if($sort['direction']){
           array_multisort($arrSort[$sort['field']], constant($sort['direction']), $n);
       }
       // foreach($n as $i=>$c){
       //     $u=M('user')->field("createtime")->where("id = $user")->find();
       //     if($c[createtime] >$u[createtime]){
