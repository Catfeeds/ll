...
{
	$sql = 'SELECT name, id FROM ' . $GLOBALS['ecs']->table('color');
	$res = $GLOBALS['db']->getAll($sql);

	$color_list = array();
	foreach ($res AS $row)
	{
		$color_list[$row['id']] = addslashes($row['name']);
	}
...
/**
 * 取得分类列表
 * @return array 分类列表 id => name
 */
function get_classify_list()
{
	$sql = 'SELECT name, id FROM ' . $GLOBALS['ecs']->table('classify');
	$res = $GLOBALS['db']->getAll($sql);

	$classify_list = array();
	foreach ($res AS $row)
	{
		$classify_list[$row['id']] = addslashes($row['name']);
	}
...
/**
 * 取得优惠券列表
 * @return array 优惠券列表 id => name
 */
function get_coupon_list($type = 0)
{
	$sql = 'SELECT name, id FROM ' . $GLOBALS['ecs']->table('coupon');
	if ( $type == 0 ) {
		$sql . " where now() <= expiration_time";
	}
	$res = $GLOBALS['db']->getAll($sql);
	...
	foreach ($res AS $row)
	{
		$coupon_list[$row['id']] = addslashes($row['name']);
	}
...
function cat_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($_POST['type'])
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('classify') ." WHERE classify_type = '$type'";
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('classify');
        }
        ...
        if ($_POST['type'])
        {
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('classify')." WHERE classify_type = '$type' ORDER BY id ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('classify')." ORDER BY id ASC";
        }
    ...
    foreach ($arr as $k=>$v) {
        switch ($v['classify_type']) {
            case 1: $arr[$k]['type_name'] = '款式'; break;
            case 2: $arr[$k]['type_name'] = '色彩'; break;
            case 3: $arr[$k]['type_name'] = '价格'; break;
            default: $arr[$k]['type_name'] = '风格'; break;
        }
    }

    return array('cat' => $arr, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
}
/**
 * 取得上次的过滤条件
