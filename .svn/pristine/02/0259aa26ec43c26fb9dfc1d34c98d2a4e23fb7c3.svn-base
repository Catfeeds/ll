...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['formated_order_amount']   = price_format($order['goods_amount']+$order['shipping_fee'], false);
 ...
 * @return  bool
 */
function update_order_actioin($order_id, $order)
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_action'),
        $order, 'UPDATE', "order_id = '$order_id'");
}
