...
class InformationController extends AdminbaseController {
    ...
    //添加
     public function add(){
         if($_GET[id]){
             $i=M('information')->where("id = $_GET[id]")->find();
             //h5
             if($i['h5_id']){
     ...
     public function add_post(){
         ...
         if(empty($editid)){
            ...
            $data=array('icon'=>$picture,'title'=>$title,'content'=>$content,'h5_id'=>$h5_id,'createtime'=>date('Y-m-d H:i:s',time()));
             ...
             $data=array('icon'=>$picture,'title'=>$title,'content'=>$content);
             if(M('information')->where("id = $editid")->save($data) || M("h5")->where("id = $h5_id")->save($data_h5)){
                 $this->success('修改成功',U('information/index'));
             }else{
                 $this->error("修改失败");
             }
         }
     ...
     public function delete(){
        ...
        if (M('information')->delete($id)!==false) {
            $this->success("删除成功！");
        } else {
            $this->error("删除失败！");
        }
