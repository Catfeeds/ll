...
if (empty($_POST['uid']) || empty($_POST['token']) || empty($_POST['goods_id']) || empty($_POST['color_id']))
{
	$val->code = 201;
	$val->msg = '缺少必要的参数';
	die(json_encode($val));
...
$val->msg = '操作成功';
//参数
$sql = "select * from ecs_cus_texture where goods_id = '$goods_id' and color_id = '$color_id'";
//分页算法
